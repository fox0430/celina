...
when hasAsyncSupport:
  ...
  suite "AsyncApp Event and Render Handlers":
    ...
    test "onRenderAsync with AsyncApp context sets render handler":
      let app = newAsyncApp()
      var handlerCalled = false
      var receivedApp: AsyncApp
    ...
    test "onRenderAsync overloads are mutually exclusive":
      let app = newAsyncApp()
    ...
    test "onRenderAsync(nil) resets handler":
      let app = newAsyncApp()
      var called = false
      app.onRenderAsync proc(buffer: var Buffer, app: AsyncApp) =
        called = true
      ...
      called = false
      let nilHandler: proc(buffer: var Buffer, app: AsyncApp) = nil
      app.onRenderAsync(nilHandler)
      app.dispatchRenderAsync()
      check called == false

    test "onRenderAsync(nil) resets simple-form handler":
      let app = newAsyncApp()
      var called = false
      app.onRenderAsync proc(buffer: var Buffer) =
        called = true
